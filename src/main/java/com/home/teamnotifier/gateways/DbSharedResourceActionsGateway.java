...
public class DbSharedResourceActionsGateway implements SharedResourceActionsGateway
{
  ...
  @Inject
  public DbSharedResourceActionsGateway(final TransactionHelper transactionHelper)
  {
  ...
  @Override public ActionsInfo getActions(Range<LocalDateTime> range)
  {
    final List<ActionOnSharedResourceEntity> actions=transactionHelper.transaction(em -> {
      ...
      final CriteriaQuery<ActionOnSharedResourceEntity> allInRange=
          cq.select(rootEntry).where(getPredicateForRange(range, cb, rootEntry)).orderBy(cb.desc(time));
    ...
    final List<ActionInfo> actionInfos=actions.stream()
        .map(a -> new ActionInfo(a.getActor().getName(), a.getActionTime().toString(), a.getDetails()))
